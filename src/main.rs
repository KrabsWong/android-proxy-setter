...
fn main() -> Result<()> {
    ...
    if !adb_version.status.success() {
        anyhow::bail!(
            "Failed to execute ADB command, please ensure ADB is installed and added to PATH"
        );
    }
    ...
    let current_proxy_setting = String::from_utf8_lossy(&current_proxy.stdout)
        .trim()
        .to_string();
...
fn set_proxy(args: &Args) -> Result<()> {
    ...
    if verify_proxy.status.success() {
        let proxy_setting = String::from_utf8_lossy(&verify_proxy.stdout)
            .trim()
            .to_string();
        ...
        println!(
            "{}",
            "⚠️ Unable to verify proxy settings, please verify manually"
                .yellow()
                .bold()
        );
    }
...
fn clear_proxy() -> Result<()> {
    ...
    if verify_proxy.status.success() {
        ...
        if proxy_setting.is_empty() || proxy_setting == ":0" {
            println!(
                "{}",
                "✅ Successfully cleared Android device proxy!"
                    .green()
                    .bold()
            );
        } else {
...
fn restart_adb() -> Result<()> {
    ...
    if status.success() {
        println!("Execute restart adb success");
    } else {
        eprintln!("Execute restart adb failed, {}", status);
    }
